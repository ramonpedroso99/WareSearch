...
todas_integracoes = []
todos_clientes = []  # Lista para armazenar todos os clientes
...
async def carregar_dados():
    ...
    global todas_integracoes, todos_clientes
    ...
    try:
        ...
        todos_clientes = rows_clientes  # Armazena todos os clientes para filtrar depois
...
def pesquisar_clientes():
    """Filtra os clientes conforme o texto digitado na pesquisa"""
    texto_pesquisa = entry_pesquisa.get().lower()  # Texto da pesquisa
    tree_clientes.delete(*tree_clientes.get_children())  # Limpa a tabela

    # Filtra clientes com base na pesquisa (nome ou código)
    clientes_filtrados = [
        cliente for cliente in todos_clientes if texto_pesquisa in cliente["nome"].lower() or texto_pesquisa in str(cliente["codigo"])
    ]

    # Insere os clientes filtrados na tabela
    for cliente in clientes_filtrados:
        tree_clientes.insert("", "end", values=(cliente["codigo"], cliente["nome"]))
...
# Barra de pesquisa
entry_pesquisa = ctk.CTkEntry(frame_clientes, placeholder_text="Pesquisar por código ou nome...", font=("Arial", 12))
entry_pesquisa.pack(pady=10, fill="x")
entry_pesquisa.bind("<KeyRelease>", lambda event: pesquisar_clientes())  # Atualiza a pesquisa a cada tecla pressionada
